...
const qaData = [
  // ─── A. 關於本工具 ───────────────────────────
  {
    ...
    link: "https://drwu.tcmdr.tw/",
    linkText: "更多資訊：阿銘醫師的個人網站"
  },
  {
    q: "本工具的數據來源與計算方法",
    a: "【數據來源】\n\n" +
       "● 0–5 歲身高體重：世界衛生組織 (WHO) 公布之「國際嬰幼兒生長標準」\n" +
       "● 7–18 歲身高體重：陳偉德醫師及張美惠醫師 2010 年發表之台灣兒少生長新曲線研究\n" +
       "● 5–7 歲銜接：參考 WHO BMI Rebound 趨勢進行數據平滑銜接\n" +
       "● BMI 體位判定：衛生福利部國民健康署公告之「台灣兒童及青少年 BMI 參考值」\n" +
       "● 骨齡預估成年身高：Bayley–Pinneau (BP) 模型 (J Pediatr, 1952;40:423-441)\n\n" +
       "【計算方法：精密分段線性內插法】\n\n" +
       "不同工具即便數據同樣參考國健署標準，若計算邏輯不同，仍會產生微小落差。本工具有三個設計重點：\n\n" +
       "● 完整還原原始論文七段基準線\n" +
       "一般工具多採簡化後的五段線（P3, P15, P50, P85, P97）估算。本工具完整調用七段基準線（加入 P25 與 P75），量尺刻度更細，能更精確定位百分位落點。\n\n" +
       "● 先定時間：精密實足年齡\n" +
       "系統依出生年月日算出到「測量當天」的實足年齡，即便原始數據為每 0.5 歲一檔，也會先推算出當天專屬的七條基準線，避免因生日邊界導致數據跳躍。\n\n" +
       "● 後定數值：分段精密內插\n" +
       "先產出當日專屬量尺，再對孩子的實測值進行分段內插。這種兩段式邏輯能減少線性誤差，完整還原真實的生長軌跡。",
    link: "https://www.pediatr-neonatol.com/article/S1875-9572(10)60014-9/pdf",
    linkText: "參考文獻：台灣兒少生長新曲線研究"
  },
  {
    q: "本工具的使用限制與免責聲明",
    a: "本工具於設計與資料層面有以下原則性限制，使用前請仔細閱讀：\n\n" +
       "● 採線性內插而非 WHO 官方 LMS 方法\n" +
       "原始論文並未公佈 L/M/S 三參數，本工具以七段線性內插逼近。對落在七條基準線附近的孩子精度已足夠；但在極端區（百分位 < 3 或 > 97）單點誤差會被放大，因此本工具在這個區間直接以「< P3」或「> P97」呈現，不顯示具體數字以避免誤讀。\n\n" +
       "● 年齡資料範圍\n" +
       "身高體重百分位資料僅涵蓋 0–18.5 歲台灣兒少；BMI 體位判定上限為 17.5 歲。超出此年齡區間的結果不具臨床意義。\n\n" +
       "● 早產兒矯正年齡\n" +
       "僅在出生後 3 歲以內的早產兒做年齡校正（符合台灣兒科醫學會共識）；超過 3 歲後改以實足年齡對照曲線。\n\n" +
       "● 遺傳身高（Tanner 公式）\n" +
       "「依父母身高推估孩子成人身高」屬於族群統計之平均期望值，個體遺傳變異幅度大，實際差距常達 ±8 cm 以上。本工具採男 ±7.5 cm、女 ±6 cm 作為「符合遺傳預期」區間（較臨床教科書常用的 ±8.5 cm 嚴格），目的為提早識別潛在偏離，並非作為最終身高預測結果。\n\n" +
       "● 骨齡預估成年身高（Bayley–Pinneau 模型）\n" +
       "此法原始樣本以歐美族群為主，對東亞兒少可能有系統性偏差；骨齡判讀本身因放射科醫師與採用方法（GP、TW3）不同也會有差異，請以實際骨齡報告的判讀為主。\n\n" +
       "● 青春期分級\n" +
       "若有輸入 Tanner 期數，以 Tanner 為主；若未輸入，只能以年齡 + 身高百分位「粗略推估」，屬初步篩選，無法取代門診理學檢查與第二性徵評估。\n\n" +
       "● 非醫療診斷工具\n" +
       "本工具為衛教輔助，不是診斷工具，也不取代骨齡片、內分泌抽血、生長激素激發試驗等臨床評估。若懷疑孩子有生長遲緩、性早熟或內分泌異常，請直接就醫。\n\n" +
       "阿銘醫師提醒：計算結果僅供參考。比起單次數值，更重要的是觀察孩子的百分位是否在短時間內「大幅改變」或「突然跨區」— 這才是評估生長是否健康的核心指標。"
  },

  // ─── B. 各項指標如何判讀 ─────────────────────
  {
    q: "早產兒為什麼要算「矯正年齡」？",
    a: "「實足年齡」是從孩子出生當天算起；「矯正年齡」則是從原訂預產期開始計算。兩者差距就是「提早出生的週數」。\n\n" +
       "● 臨床必要性\n" +
       "早產兒在 2–3 歲前的器官發育、大動作發展與身高體重，需以「矯正年齡」對照生長曲線才具參考價值，避免與足月兒比較而產生不必要的焦慮。\n\n" +
       "● 本工具設定\n" +
       "在出生後 3 歲以內提供矯正功能；一旦滿 3 歲，即強制回歸實足年齡，這也符合多數臨床追蹤的實務標準。\n\n" +
       "● 計算公式\n" +
       "提早週數 = 40 週 − (出生胎齡週數 + 天數 ÷ 7)\n" +
       "矯正年齡 = 實足年齡 − 提早週數 ÷ 52.1775\n" +
       "（52.1775 = 365.25 天 ÷ 7,即一年的週數。與其他工具若有「幾天」的微小差距屬正常。）",
  ...
  {
    q: "BMI 體位怎麼判定？",
    a: "本工具依據衛生福利部國民健康署公告之「台灣兒童及青少年 BMI 參考值」進行判定：\n\n" +
       "● 體重過輕：低於該年齡 BMI 第 5 百分位\n" +
       "● 體重適中：第 5 至第 85 百分位之間\n" +
       "● 體重過重：第 85 至第 95 百分位之間\n" +
       "● 肥胖：達到或超過第 95 百分位\n\n" +
       "【使用限制】\n\n" +
       "● BMI 僅是身高與體重的比例指標，無法區分脂肪與肌肉。愛運動、肌肉量多的孩子可能被誤判為過重。\n" +
       "● 本工具 BMI 體位判定上限為 17.5 歲，超過年齡會顯示「無法判斷」。\n" +
       "● 實際判讀仍建議結合體脂監測、飲食紀錄與醫師評估。",
  ...
  {
    q: "發育狀態提醒（青春期）怎麼判斷？",
    a: "本工具採兩層判讀機制，優先採臨床實測資料，缺資料時才退回統計推估：\n\n" +
       "【第一層：Tanner 期數判讀（優先）】\n\n" +
       "若有填入第二性徵分期（女生以乳房發育、男生以睪丸體積為主要指標），系統會依「年齡 × Tanner 期數」交叉判讀：\n\n" +
       "女孩：\n" +
       "● 8 歲前：Tanner ≥ 1 → 發育明顯提早\n" +
       "● 8–10 歲：Tanner ≥ 2 → 略為提早；Tanner = 0 → 略為偏慢\n" +
       "● 10–13 歲：Tanner = 2 為正常；Tanner ≥ 3 略為提早；Tanner = 0 → 發育明顯偏慢\n" +
       "● 13 歲後：Tanner ≤ 1 → 發育明顯延遲\n\n" +
       "男孩（時程較女孩晚約 1 歲）：\n" +
       "● 9 歲前：Tanner ≥ 1 → 發育明顯提早\n" +
       "● 9–11 歲：Tanner ≥ 2 → 略為提早；Tanner = 0 → 略為偏慢\n" +
       "● 11–14 歲：Tanner = 2 為正常；Tanner ≥ 3 略為提早；Tanner = 0 → 發育明顯偏慢\n" +
       "● 14 歲後：Tanner ≤ 1 → 發育明顯延遲\n\n" +
       "【第二層：身高百分位粗略推估（無 Tanner 資料時）】\n\n" +
       "只能用「年齡 × 身高百分位」做初步篩檢，屬於粗略推估：\n\n" +
       "● 青春期開始前（女 < 8、男 < 9）：身高 ≥ P85 → 疑似提早；≥ P97 為顯著提早\n" +
       "● 青春期主要區間（女 8–13、男 9–14）：P85–97 提早傾向；P3–15 延遲傾向\n" +
       "● 青春期主要區間後（女 > 13、男 > 14）：身高 ≤ P15 → 疑似延遲；≤ P3 為顯著延遲\n\n" +
       "此為間接推估，確切狀態仍應以臨床觸診與骨齡評估為準。",
    link: "https://my.clevelandclinic.org/health/body/puberty",
    linkText: "參考資料：Cleveland Clinic"
  },
  {
    q: "遺傳身高（TH）與預估成年身高（PAH）怎麼看？",
    a: "這兩個是評估孩子未來身高的兩條獨立軸線，各看不同面向：\n\n" +
       "【遺傳目標身高 Target Height, TH】\n\n" +
       "依父母身高推估的「遺傳潛力區間」，又稱 mid-parental height (MPH)。採 Tanner 公式：\n\n" +
       "● 男孩：TH =（父 + 母 + 13）÷ 2,本工具採 ±7.5 cm 為符合遺傳預期區間\n" +
       "● 女孩：TH =（父 + 母 − 13）÷ 2,本工具採 ±6 cm 為符合遺傳預期區間\n\n" +
       "臨床教科書常用區間為 ±8.5 cm（約 ±2 SD），本工具採用較嚴格之區間，目的為提早識別潛在偏離；實務上多數孩子最終身高會落在 TH ± 8–10 cm 之間。\n\n" +
       "【預估成年身高 Predicted Adult Height, PAH】\n\n" +
       "依「骨齡」預估剩餘生長空間，需要 X 光測出的骨齡數據。本工具採 Bayley–Pinneau (BP) 模型，自動依骨齡與實足年齡差距分類：\n\n" +
       "● 早熟（Accelerated）：骨齡超前實足年齡 ≥ 1 歲\n" +
       "● 平均（Average）：骨齡差距 < 1 歲\n" +
       "● 晚熟（Retarded）：骨齡落後實足年齡 ≥ 1 歲\n\n" +
       "比起單純看實足年齡，結合骨齡能更精準反映個體發育動態與剩餘生長空間。\n\n" +
       "【BP 模型對台灣兒童的適用性】\n\n" +
       "BP 是國際內分泌科通用標準，但原始樣本以歐美族群為主，對台灣兒少可能有系統性偏差。臨床判讀建議：\n\n" +
       "1. 將 PAH 作為「趨勢參考」而非絕對預言\n" +
       "2. 與 TH 交叉比對（本工具即採此設計）\n" +
       "3. 偏離過大時由醫師結合骨齡判讀方法（GP、TW3）與內分泌評估綜合決定"
  },
  {
    q: "生長曲線偏離遺傳基準怎麼看？",
    a: "本工具提供兩個獨立的「偏離偵測」機制，分別看不同時間軸：\n\n" +
       "【軸線 A:目前百分位 vs 遺傳百分位】（任何時候都會算）\n\n" +
       "比對「孩子目前的身高百分位」與「根據父母身高推算的遺傳目標百分位」，落差超過 15 個百分位即觸發提醒：\n\n" +
       "● 落差 > +15:生長表現高於遺傳預期\n" +
       "　可能生長速度較快；建議結合骨齡與第二性徵，排除性早熟或骨骼過早成熟。\n" +
       "● 落差 < −15:生長表現低於遺傳預期\n" +
       "　目前生長動能較弱；建議檢視睡眠、蛋白質與熱量攝取、腸胃吸收。若百分位持續下滑，建議就醫。\n\n" +
       "【軸線 B:骨齡預估身高 (PAH) vs 遺傳身高範圍 (TH)】（有輸入骨齡時才啟用）\n\n" +
       "比對「預估的未來成年身高」與「遺傳目標身高區間」：\n\n" +
       "● 明顯高於預期：PAH > TH 上限 + 5 cm\n" +
       "　需排除骨齡過度超前或性早熟導致生長板提早閉合。\n" +
       "● 高於遺傳範圍：PAH > TH 上限(未達 +5 cm)\n" +
       "　生長表現優於遺傳基準，持續監測發育節奏。\n" +
       "● 符合遺傳預期：PAH 落在 TH 範圍內 → 穩定。\n" +
       "● 略低於預期：PAH < TH 下限(未達 −5 cm)\n" +
       "　檢視蛋白質、睡眠、運動強度。\n" +
       "● 明顯低於預期：PAH < TH 下限 − 5 cm\n" +
       "　需警覺是否有生長激素分泌、營養吸收障礙或慢性因素，建議由專業醫師進行內分泌與骨齡進階評估。\n\n" +
       "兩條軸線互為補充：A 看「現在的相對位置」，B 看「未來的預估落點」。若兩條軸線都偏離，警訊權重加大。"
  },

  // ─── C. 家長行動指南 ────────────────────────
  {
    q: "孩子一年應該長幾公分？",
    a: "身高百分位看的是「位置」，年生長速度（Growth Velocity）看的是「動能」。位置可能因遺傳天生偏矮、但動能正常，這就不算異常；反之，位置正常但動能突然下降，才是該警覺的訊號。\n\n" +
       "【各年齡段正常年生長速率（臨床參考）】\n\n" +
       "● 出生 – 6 個月：約 16–17 cm / 半年\n" +
       "● 6 – 12 個月：約 8 cm / 半年\n" +
       "● 1 – 2 歲：約 10 cm / 年\n" +
       "● 2 – 3 歲：約 8 cm / 年\n" +
       "● 3 歲 – 青春期前：約 5–7 cm / 年\n" +
       "● 青春期高峰（女 10–12、男 12–14）：女約 8 cm / 年，男約 9–10 cm / 年\n" +
       "● 青春期後：逐年減速至生長板閉合\n\n" +
       "【警訊門檻】\n\n" +
       "● 3 歲以後若年生長速度 < 4 cm,或連續 6 個月身高百分位下降超過一個區段(例：50th 掉到 25th),建議就醫評估。\n" +
       "● 青春期啟動後若年生長速度仍 < 4 cm,需留意是否骨齡過度超前、生長板已接近閉合。\n\n" +
       "阿銘醫師提醒：相較於單次測量，建議每 3–6 個月於固定時間、以同一量尺、由同一人操作量測一次身高，並完整記錄以便觀察長期趨勢。單次偏離多屬測量誤差，唯有持續性的趨勢偏離才具臨床參考意義。"
  },
  {
    q: "什麼時候該安排「骨齡（Bone Age）」檢查？",
    a: "「實足年齡」是出生證明上的數字；「骨齡」則反映生長板成熟度的「生理時鐘」。骨齡像「看油表」，讓我們知道成長空間還剩多少。\n\n" +
       "阿銘醫師建議，出現以下四種情況最值得安排檢查：\n\n" +
       "● 生長軌跡偏離：曲線突然跨區（例如從 50th 掉到 15th,或突然衝高），代表生長動能不尋常。\n" +
       "● 身高極端：身高低於第 3 百分位(偏矮)或高於第 97 百分位(偏高)。\n" +
       "● 發育時鐘提早：女孩 8 歲前、男孩 9 歲前出現第二性徵，需排除性早熟導致生長板提早閉合。\n" +
       "● 預測終身高：想結合遺傳目標(TH)更精準地評估未來成年身高。\n\n" +
       "骨齡檢查屬於低劑量 X 光(約等於幾小時日常背景輻射量),必要時做並不需要過度擔憂；但也不需要頻繁拍攝，建議由醫師評估後於關鍵轉折點檢查即可。"
  },
  {
    q: "看到系統跳出偏離提醒，家長該怎麼做？",
    a: "系統顯示偏離提醒時，並不直接代表孩子有疾病，而是提示家長需要進一步關注孩子的生長節奏。建議依以下三步驟進行評估：\n\n" +
       "【第一步：觀察趨勢】\n\n" +
       "單次偏離可能源自測量誤差或短期波動。建議每 3–6 個月定期追蹤，觀察生長曲線是否持續偏離原有軌跡。若下一次測量回到原本區間，通常屬於正常生理變動。\n\n" +
       "【第二步：檢視生活】\n\n" +
       "請檢視孩子近期的：\n" +
       "● 睡眠：是否晚於 10 點入睡？深眠期正是生長激素分泌高峰。\n" +
       "● 營養：優質蛋白質(蛋、魚、肉、乳製品)、鈣、鋅是否足夠？\n" +
       "● 運動：是否有規律運動(尤其是縱向刺激如跳繩、籃球)?\n" +
       "● 情緒與壓力：長期心理壓力也會影響生長激素。\n\n" +
       "【第三步：專業諮詢】\n\n" +
       "若連續兩次測量皆偏離，或年生長速度低於 4 公分(3 歲以後),建議與專業醫師討論，可能安排：\n" +
       "● 骨齡 X 光\n" +
       "● 甲狀腺功能、生長激素、第二性徵檢查\n" +
       "● 中醫體質辨證與調理\n\n" +
       "阿銘醫師提醒：面對系統顯示的偏離提醒，最常見的誤區是家長急於自行查找網路資料或自行進補，反而延誤判讀時機。建議家長先定期追蹤生長趨勢，並於必要時尋求專業醫師評估，以免錯失介入時機或產生不必要的焦慮。"
  },
  {
    q: "中醫在兒童生長發育中扮演什麼角色？",
    a: "中醫看兒童生長，不只看身高數字，更看全身臟腑氣血的平衡表現。介入重點在於「調和脾胃」與「補益腎氣」，特別針對以下情況有顯著幫助：\n\n" +
       "● 脾胃運化\n" +
       "針對食慾不振、挑食、吸收不良的孩子，透過調理脾胃，讓營養能有效轉化為成長動能。\n\n" +
       "● 體質干擾\n" +
       "過敏性鼻炎、異位性皮膚炎、睡眠品質不佳、壓力過大等因素皆可能影響生長激素分泌。中醫透過體質調理改善上述干擾因子，協助孩子在較佳的生理狀態下發育。\n\n" +
       "● 轉骨時機\n" +
       "中醫講究「因材施教」，針對不同發育階段給予個人化轉骨方，而非盲目進補。透過客觀數據(身高百分位、骨齡、Tanner 期)監測，能精準掌握介入的黃金期。\n\n" +
       "阿銘醫師提醒：中、西醫並無衝突，合作更能提供完整的兒童健康照護。西醫擅長結構性評估與急性介入，中醫則著重整體調理與體質優化；兩者互補運用，有助於協助孩子在穩定的生理條件下完成發育。"
  }
];
